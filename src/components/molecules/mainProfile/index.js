    ...
    useEffect( () => {
        const fetchData = async () => {
            if(isKelas !== undefined){
                ...
                await Axios.get(`https://pangasari.herokuapp.com/user/dashboards/${kelas}?page=${count}&perPage=5`)
                    .then(async user => {
                    ...
                    <div>
                        {
                            ...
                            user.map((data,index) => {
                                return <ListTeman listFriends={data} Stylee={value === array[0]? "block" : "hidden" }nilaiQuizz={array}data={dataId} mobileStyle={arrayForMobile.indexOf(arrayForMobile[0]).toString() === value ? "block" : "hidden"} checkMobile={arrayForMobile} key={dataId}/>
                            })
                        }
                    </div>
