...
const Login = () => {
    ...
    const inRefresh = (refreshToken) => {
        return new Promise((resolve,reject) => {
            Axios.post('https://pangasari.herokuapp.com/refresh', {token: refreshToken})
            .then(respone => {
        ...
        return new Promise((resolve,reject) => {
            ...
            .then(async respone => {
                if(respone.data.success === false){
                    ...
                    window.location.assign(`/user/dashboard/${user.firstName+"_"+user.lastName.split(" ").filter(e => e).join("_")}`)
                }
            })
        })
    }
